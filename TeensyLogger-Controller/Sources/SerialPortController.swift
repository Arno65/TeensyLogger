...
//
// (cl)  Spring, Summer, Autumn 2022 ... spring 2026  by Arno Jacobs
//
...
//  version 1.9.2x  2023-11-14      After exact one year of maintenance...
//                                  The previous version did NOT work 0K on MacOS Sonoma.
//                                  Workaround at function initValues() around line 325: (px,py) = (720,720)
//  version 1.9.3p  2025-04-02      Changed ORSSerialPort setup, the Package Dependencies now via Git.
//  version 1.9.4p  2026-04-15      The mouse click on the plot didn't work - probably since the MacOS Sonoma version (2023).
//                                  Solved it by adding a transparent button over the plot.
//                                  Minimum deployment is still set on MacOS 11. This version is only tested on MacOS Tahoe.
//
...
class SerialPortController: NSObject, NSApplicationDelegate, ORSSerialPortDelegate {
    ...
    static let shared = SerialPortController()
    ...
    func initValues() {
        ...
        px = 720 // canvas.visibleRect.width        // WAS - Get the canvas size
        py = 720 // canvas.visibleRect.height       // NOW: FIXED at (720 by 720)
        ...
        if needsInit {
            ...
            loopRun.state   = NSControl.StateValue.off // was  .on
            checkLoopArm    = false
            checkLoopRun    = false
    ...
    //
    // 2026-04-15
    // In older versions of this code this next @IBAction function was coupled to the 'Image View' / 'Image Cell'
    // They where clickable
    // Later with Mac OS Sonoma / Tahoe this stop working
    // I solved it with placing a transparent huge square button 'Plot Click invisible button', covering the complete image
    // It's working again.
    //
