...
stream          Start continuous data acquisition\n\
...
void sample() {
    ...
    for (unsigned i = 0; i < active_channels; i++) data[next_sample][i] = 0;
...
void loop() {
    ...
    if (Serial.available() > 0) {       // There is something to read and process
        ...
        // Stream mode ... 
        // do 1. 'arm'
        //    2. one sample for all selected channels
        //    3. display one line with all acquired data
        //    4. check for 'stop' ?? -- or trigger pulse ??? -- if so then END
        //    5. goto 2.
        // 
        if (!strcmp(command, "stream")) {
            Serial.print("\n* Stream mode NOT implented yet! *");
        } else
